...
class TaskHandler:
    ...
    def create_task(cursor: Cursor, title: str, text: str, user_id: int, status: str = 'Created') -> Task:
        ...
        cursor.execute(f"""INSERT INTO tasks (title, text, timestamp, status, user_id) VALUES ('{
                       title}', '{text}', {timestamp}, '{status}', {user_id})""")
    ...
    def get_task_by_id(cursor: Cursor, task_id) -> Task | None:
        cursor.execute(
            f"SELECT rowid, title, text, timestamp, status, user_id FROM tasks WHETE rowid={task_id}")
    ...
    def get_list(cursor: Cursor, user_id: int) -> tuple[Task, ...] | None:
        cursor.execute(
            f"SELECT rowid, title, text, timestamp, status FROM tasks WHERE user_id={user_id}")
